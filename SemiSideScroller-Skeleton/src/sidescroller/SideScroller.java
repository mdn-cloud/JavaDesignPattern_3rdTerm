...
public class SideScroller extends Application{
	...
	public void init() throws Exception{
		...
		board.setAnimator(animator); //1.7
		
		PlayerInput input = new PlayerInput (new InputAdapter(canvas)); //1.8
		
		PlayerSprite ps = new PlayerSprite(); //1.9
		ps.setScale(SCALE);    //1.10
		
		Player player = new Player(width/2, height/5, 20*SCALE, 20*SCALE, ps);  //1.11
		player.setInput(input);  //1.12
		
		List<Entity> players = board. players(); //1.13 & 1.14
		players.add(player);   //1.15
		
		Region statusBar = createStatusBar(); //1.16
		Region optionsBar = createOptionsBar(); //1.17
		
		root = new BorderPane(); //1.18
		root.setTop(optionsBar); //1.19
		root.setCenter(canvas); //1.20
		root.setBottom(statusBar); //1.21
	...
	private Region createOptionsBar(){
		...
		CheckMenuItem grid = createCheckMenuItem( "Grid", true, board.drawGridProperty());
	...
	public CheckMenuItem createCheckMenuItem( String text, boolean isSelected, BooleanProperty binding){
		...
		// bind BooleanProperty "binding" which uses observer pattern to check.selectedProperty(). use the bind method in "binding"
		// use the method CheckMenuItem::setSelected and pass isSelected to it.
		
		CheckMenuItem cmi = new CheckMenuItem(text);
		binding.bind(cmi.selectedProperty());
		cmi.setSelected(isSelected);
		return cmi;
	}
